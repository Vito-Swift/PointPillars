...
def anchor_target_single_class(batched_anchors, batched_gt_bboxes, batched_gt_labels, assigners):
    batch_size = len(batched_anchors)
    for i in range(batch_size):
        anchors = batched_anchors[i][:, :, 0, :, :].reshape(-1, 7)  # only use the first anchor (2 * x_l * y_l, 7)
        # 1 (single class), x_l, y_l (feature map), 2, 7
        d1, d2, d3, d4, d5 = batched_anchors[i].size()
        pos_iou_thr, neg_iou_thr, min_iou_thr = \
            assigners[0]['pos_iou_thr'], assigners[0]['neg_iou_thr'], assigners[0]['min_iou_thr']

        # select i-th ground truth bbox
        gt_bboxes = batched_gt_bboxes[i]
        multi_bbox_reg, multi_bbox_reg_weights = [], []
        multi_dir, multi_dir_weights = [], []

        # 1. calculate overlap between anchors and each ground truth bounding box
        # overlaps.shape = (n_bbox, 2 * x_l * y_l, dtype=float)
        overlaps = iou2d_nearest(gt_bboxes, anchors)
        # max_overlaps.shape = (2 * x_l * y_l)
        # maximal overlaps between anchors and indifferent ground truth bounding box
        max_overlaps, max_overlaps_idx = torch.max(overlaps, dim=0)
        # gt_max_overlaps.shape = (n_bbox)
        gt_max_overlaps, _ = torch.max(overlaps, dim=1)

        # 2. classify anchors
        # assigned_gt_inds.shape = (2 * x_l * y_l)
        assigned_gt_inds = -torch.ones_like(anchors[:, 0], dtype=torch.long)
        # negative anchors
        assigned_gt_inds[max_overlaps < neg_iou_thr] = 0
        # positive anchors
        # if the overlap exceeds the positive iou threshold, assigned it to the index of the corresponding bounding box
        ...
        # anchor regression
        assigned_gt_reg_weights = torch.zero_like(anchors[:, 0])
        assigned_gt_reg_weights[pos_flag] = 1

        assigned_gt_reg = torch.zero_like(anchors)
        positive_anchors = anchors[pos_flag]
        ...
        # anchor direction
        assigned_gt_dir_weights = torch.zero_like(anchors[:, 0])
        assigned_gt_dir_weights[pos_flag] = 1
        assigned_gt_dir = torch.zero_like(anchors[:, 0], dtype=torch.long)
        dir_targets = limit_period(corr_gt_bboxes[:, 6].cpu(), 0, 2 * np.pi).to(corr_gt_bboxes)
        dir_targets = torch.floor(dir_targets / np.pi).long()
        assigned_gt_dir[pos_flag] = torch.clamp(dir_targets, min=0, max=1)
        ...
        multi_dir.append(assigned_gt_dir.reshape(d1, d2, 1, d4))
        multi_dir_weights.append(assigned_gt_dir_weights.reshape(d1, d2, 1, d4))
